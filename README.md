...
[MCPDoc](https://github.com/langchain-ai/mcpdoc) is a tool that serves documentation using the Model Context Protocol (MCP). It allows you to create documentation servers that can be consumed by LLM applications, providing structured access to documentation from various sources including `llms.txt` files, YAML configurations, and direct URLs.

This daemon provides automated monitoring and reloading capabilities for mcpdoc servers, making it easy to maintain up-to-date documentation services in containerized environments.
...
```bash
python -m mcpdoc.cli --yaml /config/config.yaml --urls "fastapi:/config/fastapi.llms.txt" "something:/config/something.llms.txt" --follow-redirects --timeout 10.0 --allowed-domains '*' --transport sse --host 0.0.0.0 --port 8080 --log-level INFO
```
